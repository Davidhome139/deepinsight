...
// GetPostgresConfig 获取数据库配置，确保从环境变量加载值
func GetPostgresConfig() *PostgresConfig {
	// 直接从环境变量中获取数据库配置
	dbName := GetEnvWithDefault("APP_DATABASE_POSTGRES_DBNAME", "yuanbao")
	// 确保数据库名称不为空
	if dbName == "" {
		dbName = "yuanbao"
	}
	return &PostgresConfig{
		Host:     GetEnvWithDefault("APP_DATABASE_POSTGRES_HOST", "db"),
		Port:     5432,
		User:     GetEnvWithDefault("APP_DATABASE_POSTGRES_USER", "postgres"),
		Password: GetEnvWithDefault("APP_DATABASE_POSTGRES_PASSWORD", ""),
		DBName:   dbName,
		SSLMode:  GetEnvWithDefault("APP_DATABASE_POSTGRES_SSLMODE", "disable"),
	}
}
...
func LoadConfig(path string) (*Config, error) {
	...
	// 打印配置内容，用于调试
	log.Printf("Config content: %+v", v.AllSettings())
	...
	// 打印反序列化后的配置，用于调试
	log.Printf("Deserialized config: %+v", config)
...
func syncConfigToViper() {
	...
	viper.Set("tools", GlobalConfig.Tools)
}
...
func GetConfigDir() string {
	...
	// Assuming configPath is something like "config/config.yaml"
	// We want to return "config"
	dir := configPath
