...
func (h *AIChatHandler) GetMCPTools(c *gin.Context) {
	...
	for serverName, server := range servers {
		if !server.Connected && server.Client == nil {
			...
			// But include built-in servers and context7 (we know its tools)
			if serverName != "terminal" && serverName != "search" && serverName != "code-analysis" && serverName != "filesystem-local" && serverName != "context7" {
				continue
			}
			
			// For context7, add known tools even if not connected
			if serverName == "context7" && len(server.Tools) == 0 {
				// Add placeholder tools for context7
