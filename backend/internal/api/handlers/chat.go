...
func (h *ChatHandler) StreamChat(c *gin.Context) {
	...
	// 调试日志：打印收到的配置
	if req.PromptEngineeringConfig != nil {
		fmt.Printf("[DEBUG] Received PromptEngineeringConfig:\n")
		fmt.Printf("[DEBUG]   Enabled: %v\n", req.PromptEngineeringConfig.Enabled)
		fmt.Printf("[DEBUG]   OutputFormat: %s\n", req.PromptEngineeringConfig.OutputFormat)
		fmt.Printf("[DEBUG]   ChainOfThought: %v\n", req.PromptEngineeringConfig.ChainOfThought)
		fmt.Printf("[DEBUG]   ToolCalls: %v\n", req.PromptEngineeringConfig.ToolCalls)
		fmt.Printf("[DEBUG]   SelfEvaluation: %v\n", req.PromptEngineeringConfig.SelfEvaluation)
		fmt.Printf("[DEBUG]   PromptOptimizationEnabled: %v\n", req.PromptEngineeringConfig.PromptOptimizationEnabled)
	} else {
		fmt.Printf("[DEBUG] PromptEngineeringConfig is nil\n")
	}
...
func (h *ChatHandler) GetMCPTools(c *gin.Context) {
	...
	for name, server := range mcps.Servers {
		...
		if server.Type == "builtin" {
			...
			// Add actual tools if we know them, otherwise add placeholder
			if name == "context7" {
				...
				toolList = append(toolList, map[string]interface{}{
					...
					"name":        "[context7] Query Documentation",
					"server":      "context7",
					"tool":        "query-docs",
				...
				// For other external servers, we don't add generic tools
				// This prevents the "call_tool not found" error
				log.Printf("[Chat] External MCP server %s not yet connected, skipping tools", name)
			}
		}
	}
...
type StreamChatRequest struct {
	...
	PromptEngineeringConfig *ai.PromptEngineeringConfig `json:"promptEngineeringConfig,omitempty"`
}
