...
// ValidateConfig validates MCP server configuration
func (h *AgentHandler) ValidateConfig(c *gin.Context) {
	...
	// 获取MCP管理器
	mcpManager := h.orchestrator.GetMCPManager()
	if mcpManager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "MCP manager not available"})
		return
	}

	// 获取服务器配置
	server, exists := mcpManager.GetServer(serverName)
	...
	// 执行验证
	validator := mcpManager.GetConfigValidator()
	if validator == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "config validator not available"})
		return
	}
...
// GetValidationHistory gets validation history for a server
func (h *AgentHandler) GetValidationHistory(c *gin.Context) {
	serverName := c.Param("server")
	if serverName == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "server name required"})
		return
	...
	history := validator.GetValidationHistory(serverName)
...
func (h *AgentHandler) GetValidationStats(c *gin.Context) {
	// 获取MCP管理器
	mcpManager := h.orchestrator.GetMCPManager()
	if mcpManager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "MCP manager not available"})
	...
	// 获取验证统计
	stats := monitor.GetValidationStats()
	...
	if validator != nil {
		...
		stats["totalCacheEntries"] = len(cacheStats)
	}
