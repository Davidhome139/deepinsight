...
type SettingHandler struct {
	...
	automationCoordinator mcp.AutomationCoordinator
}

func NewSettingHandler(coordinator mcp.AutomationCoordinator) *SettingHandler {
...
func (h *SettingHandler) UpdateMCPServer(c *gin.Context) {
	...
	// 移除冲突的type字段（配置项类型），保留server_type（服务器实际类型）
	if rawData["type"] == "mcpservers" {
		delete(rawData, "type")
	}

	// 直接处理前端发送的嵌套数据结构：{mcpServers: {context7: {command: "npx", args: [...]}}}
	var serverData map[string]interface{} = make(map[string]interface{})

	log.Printf("UpdateMCPServer: rawData keys: %+v", reflect.ValueOf(rawData).MapKeys())

	// 检查是否有mcpServers字段（大写S）
	if mcpServersVal, ok := rawData["mcpServers"]; ok {
		log.Printf("UpdateMCPServer: Found mcpServers, type: %T", mcpServersVal)
		if mcpServers, ok := mcpServersVal.(map[string]interface{}); ok {
			log.Printf("UpdateMCPServer: mcpServers content: %+v", mcpServers)
			if serverConfig, ok := mcpServers[serverKey]; ok {
				log.Printf("UpdateMCPServer: Found serverConfig in mcpServers, type: %T", serverConfig)
				if configMap, ok := serverConfig.(map[string]interface{}); ok {
					serverData = configMap
					log.Printf("UpdateMCPServer: Extracted serverData from mcpServers: %+v", serverData)
				}
			}
		}
	}

	// 如果还没有提取到数据，检查mcpservers字段（小写s）
	if len(serverData) == 0 {
		if mcpserversVal, ok := rawData["mcpservers"]; ok {
			log.Printf("UpdateMCPServer: Found mcpservers, type: %T", mcpserversVal)
			if mcpservers, ok := mcpserversVal.(map[string]interface{}); ok {
	...
	// 如果还没有提取到数据，检查是否有直接的配置字段
	if len(serverData) == 0 {
		log.Printf("UpdateMCPServer: No nested structure found, checking direct fields in rawData")
		// 检查rawData是否直接包含配置字段
		if _, hasCommand := rawData["command"]; hasCommand {
			log.Printf("UpdateMCPServer: Found direct command field, using rawData as serverData")
			serverData = rawData
		} else {
			log.Printf("UpdateMCPServer: No valid configuration found in rawData")
		}
	}

	log.Printf("UpdateMCPServer: Final serverData for %s: %+v", serverKey, serverData)

	log.Printf("UpdateMCPServer: Received rawData: %+v", rawData)
	log.Printf("UpdateMCPServer: rawData contains enabled field: %v", rawData["enabled"] != nil)
	if rawData["enabled"] != nil {
		log.Printf("UpdateMCPServer: enabled value: %v (type: %T)", rawData["enabled"], rawData["enabled"])
	}

	// 获取现有服务器配置
	mcpsConfig := config.GetMCPServersConfig()
	existingServer, exists := mcpsConfig.Servers[serverKey]
	if !exists {
		// 如果服务器不存在，创建一个新的
		existingServer = config.MCPServer{
			Name:    serverKey,
			Enabled: true,
			Type:    "command", // 默认类型
		}
	...
	// 如果serverData为空，但rawData包含enabled字段，则直接使用rawData
	if len(serverData) == 0 && rawData["enabled"] != nil {
		log.Printf("UpdateMCPServer: Using rawData directly for enabled field")
		serverData = rawData
	}

	// 使用字段映射归一化数据
	normalizedData, err := config.NormalizeMap(serverData, config.MCPServer{})
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Failed to normalize data: " + err.Error()})
		return
	}
	...
	// 将新数据合并到现有配置中
	if err := config.DecodeWithFieldMapping(normalizedData, &existingServer); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Failed to decode data: " + err.Error()})
		return
	}

	// 验证配置前添加调试日志
	log.Printf("UpdateMCPServer: Normalized data: %+v", normalizedData)
	log.Printf("UpdateMCPServer: Merged server: %+v", existingServer)
	log.Printf("UpdateMCPServer: Server Type: '%s'", existingServer.Type)
	log.Printf("UpdateMCPServer: Server Command: '%s'", existingServer.Command)

	// 验证配置
	validator := config.NewConfigValidator()
	if !validator.ValidateMCPServer(serverKey, existingServer) {
		log.Printf("UpdateMCPServer: Validation errors: %v", validator.GetErrors())
		c.JSON(http.StatusBadRequest, gin.H{
			"error":   "Validation failed",
			"details": validator.GetErrors(),
		})
	...
	config.UpdateMCPServer(serverKey, existingServer)
	log.Printf("Updated MCP server: %s", serverKey)

	// 如果服务器启用且支持自动化，触发自动化流程
	if existingServer.Enabled && h.automationCoordinator != nil {
		go func() {
			log.Printf("UpdateMCPServer: Starting automation for server: %s", serverKey)

			// 尝试从MCP注册表中获取包信息
			registry, err := config.LoadMCPRegistry()
			if err != nil {
				log.Printf("UpdateMCPServer: Failed to load MCP registry: %v", err)
				return
			}

			// 查找服务器对应的包信息
			if serverInfo, exists := registry.MCPRegistry.Servers[serverKey]; exists {
				// 根据包类型确定依赖类型
				var depType mcp.DependencyType
				switch serverInfo.PackageType {
				case "npm":
				...
				default:
					log.Printf("UpdateMCPServer: Unknown package type: %s", serverInfo.PackageType)
					return
				}

				// 触发自动化流程
				if err := h.automationCoordinator.UpdateMCPPackage(serverKey, depType); err != nil {
					log.Printf("UpdateMCPServer: Failed to start automation for %s: %v", serverKey, err)
				} else {
					log.Printf("UpdateMCPServer: Automation started for %s", serverKey)
				}
			} else {
				log.Printf("UpdateMCPServer: Server %s not found in MCP registry, skipping automation", serverKey)
			}
		}()
	}
...
func (h *SettingHandler) DeleteMCPServer(c *gin.Context) {
	...
	// 如果支持自动化，触发自动化清理
	if h.automationCoordinator != nil {
		go func() {
			log.Printf("DeleteMCPServer: Starting automation cleanup for server: %s", serverKey)
			if err := h.automationCoordinator.RemoveMCPPackage(serverKey); err != nil {
				log.Printf("DeleteMCPServer: Failed to cleanup automation for %s: %v", serverKey, err)
			} else {
				log.Printf("DeleteMCPServer: Automation cleanup completed for %s", serverKey)
			}
		}()
	}
...
func (h *SettingHandler) AddMCPServer(c *gin.Context) {
	// 读取原始 JSON 数据
	var rawData map[string]interface{}
	if err := c.ShouldBindJSON(&rawData); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid JSON: " + err.Error()})
	...
	// 获取服务器key
	serverKey, ok := rawData["key"].(string)
	if !ok || serverKey == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Server key is required"})
		return
	}
	...
	log.Printf("AddMCPServer: Received data for %s: %+v", serverKey, rawData)
	...
	config.AddMCPServer(serverKey, req)
	log.Printf("Added MCP server: %s", serverKey)

	// 如果服务器启用且支持自动化，触发自动化流程
	if req.Enabled && h.automationCoordinator != nil {
		go func() {
			log.Printf("AddMCPServer: Starting automation for server: %s", serverKey)

			// 尝试从MCP注册表中获取包信息
			registry, err := config.LoadMCPRegistry()
			if err != nil {
				log.Printf("AddMCPServer: Failed to load MCP registry: %v", err)
				return
			}
			...
			if serverInfo, exists := registry.MCPRegistry.Servers[serverKey]; exists {
				...
				case "docker":
					depType = mcp.DependencyTypeDocker
				default:
					log.Printf("AddMCPServer: Unknown package type: %s", serverInfo.PackageType)
					return
				}

				// 触发自动化流程
				if err := h.automationCoordinator.AddMCPPackage(serverKey, depType); err != nil {
					log.Printf("AddMCPServer: Failed to start automation for %s: %v", serverKey, err)
				} else {
					log.Printf("AddMCPServer: Automation started for %s", serverKey)
				}
			} else {
				log.Printf("AddMCPServer: Server %s not found in MCP registry, skipping automation", serverKey)
			}
		}()
	}
...
func testOpenAICompatible(key string, provider config.ModelProvider) gin.H {
	// 对于 zhipu、doubao 和 moonshot AI，使用各自配置的端点进行测试
	if key == "zhipu" || key == "doubao" || key == "moonshot" {
		url := provider.BaseURL + provider.Endpoint
		if provider.Endpoint == "" {
			url = provider.BaseURL + "/chat/completions"
		}

		// 构造测试请求，使用标准的 chat completions 格式
		...
		// 对于这些 AI 服务，即使返回 400 也可能是正常的（因为模型参数问题），只要认证通过就行
		if resp.StatusCode == http.StatusOK || resp.StatusCode == http.StatusBadRequest {
			var respData map[string]interface{}
			if err := json.Unmarshal(body, &respData); err == nil {
				if _, hasError := respData["error"]; hasError {
					errMap := respData["error"].(map[string]interface{})
					if code, ok := errMap["code"].(string); ok && (code == "Unauthorized" || code == "AuthenticationError") {
	...
	// 对于其他 OpenAI 兼容的 API，使用 models 端点测试
	url := provider.BaseURL
	// 检查 baseURL 是否已经以 /v1 结尾
