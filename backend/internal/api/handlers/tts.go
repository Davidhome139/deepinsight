...
func (h *TTSHandler) Speak(c *gin.Context) {
	...
	log.Printf("TTS request: text=%s, voice=%s, model=%s, speed=%f", req.Text, req.Voice, req.Model, req.Speed)

	audioData, err := h.service.Speak(req.Text, req.Voice, req.Model, req.Speed)
	if err != nil {
		log.Printf("TTS service error: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	log.Printf("TTS service success, audio data length: %d", len(audioData))
