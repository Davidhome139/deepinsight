...
func SetupRoutes(r *gin.Engine, authHandler *handlers.AuthHandler, chatHandler *handlers.ChatHandler, settingHandler *handlers.SettingHandler, videoHandler *handlers.VideoHandler, agentHandler *handlers.AgentHandler, aiChatHandler *handlers.AIChatHandler, aiChatWS *handlers.AIChatWebSocket, analyticsHandler *handlers.AnalyticsHandler, ttsHandler *handlers.TTSHandler, imageHandler *handlers.ImageHandler, ragHandler *handlers.RAGHandler, promptTemplateHandler *handlers.PromptTemplateHandler, branchHandler *handlers.BranchHandler, agentSystemHandler *handlers.AgentSystemHandler, mcpAutomationHandler *handlers.MCPAutomationHandler, jwtSecret string) {
	...
	v1 := r.Group("/api/v1")
	{
		...
		chat := v1.Group("/chat")
		{
			...
			chat.GET("/conversations/:id/prompts", prompthandlers.GetPromptFiles)
			chat.GET("/conversations/:id/prompts/:fileName", prompthandlers.GetPromptFile)
		...
		settings := v1.Group("/settings")
		{
			...
			// MCP Servers
			settings.POST("/mcpservers", settingHandler.AddMCPServer)
		...
		v1.GET("/mcp/validation/:server", agentHandler.ValidateConfig)
		v1.GET("/mcp/validation/:server/history", agentHandler.GetValidationHistory)
		v1.GET("/mcp/validation/stats", agentHandler.GetValidationStats)
		...
		// MCP Automation routes
		mcpAutomation := v1.Group("/mcp-automation")
		{
			mcpAutomation.POST("/add", mcpAutomationHandler.AddMCPPackage)
			mcpAutomation.DELETE("/remove", mcpAutomationHandler.RemoveMCPPackage)
			mcpAutomation.PUT("/update", mcpAutomationHandler.UpdateMCPPackage)
			mcpAutomation.GET("/status/:packageName", mcpAutomationHandler.GetPackageStatus)
			mcpAutomation.GET("/status", mcpAutomationHandler.GetAutomationStatus)
			mcpAutomation.POST("/start", mcpAutomationHandler.StartAutomation)
			mcpAutomation.POST("/stop", mcpAutomationHandler.StopAutomation)
		}
	}
}
