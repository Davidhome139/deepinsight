...
import (
	...
	"backend/internal/pkg/crossplatform"
)
...
func (a *ExecutorAgent) executeCommand(ctx *TaskContext, command string) (*CommandResult, error) {
	// Normalize the command and paths
	normalizedCommand := crossplatform.NormalizePath(command)

	// Use cross-platform command execution
	cpResult, err := crossplatform.ExecuteCommand(
		ctx.Context,
		normalizedCommand,
		ctx.WorkingDir,
		a.buildEnvironment(ctx),
	)
...
func (a *ExecutorAgent) buildEnvironment(ctx *TaskContext) []string {
	return crossplatform.BuildEnvironment(ctx.WorkingDir)
}
...
func getFileExtension(path string) string {
	getPath := crossplatform.NormalizePath(path)
	for i := len(getPath) - 1; i >= 0; i-- {
		if getPath[i] == '.' {
			return getPath[i:]
		}
		if getPath[i] == os.PathSeparator {
			break
		}
	}
	return ""
