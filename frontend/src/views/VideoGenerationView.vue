<template>
  <div class="video-generation">
    ...
    <header class="page-header">
      <div class="header-left">
        <el-button text @click="$router.push('/')">
          <el-icon><ArrowLeft /></el-icon> {{ t('videoGeneration.back') }}
        </el-button>
        <h1>{{ t('videoGeneration.videoGeneration') }}</h1>
      </div>
      <div class="header-right">
        <el-button @click="$router.push('/image')">{{ t('videoGeneration.images') }}</el-button>
        <el-button @click="$router.push('/ai-chat')">{{ t('videoGeneration.aiChat') }}</el-button>
      </div>
    </header>
    ...
    <main class="page-content">
      ...
      <aside class="generation-panel">
        <div class="panel-header">
          ...
          <span>{{ t('videoGeneration.createVideo') }}</span>
        </div>

        <el-form :model="form" label-position="top" class="generation-form">
          <!-- Provider Selection -->
          <el-form-item :label="t('videoGeneration.provider')">
          ...
          <!-- Model Selection -->
          <el-form-item :label="t('videoGeneration.model')">
            <el-select v-model="form.model">
              ...
              >
                <div class="model-option">
                  <span>{{ m.name }}</span>
                  <el-tag size="small" type="info">{{ m.max_duration }}s {{ t('videoGeneration.max') }}</el-tag>
                </div>
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <!-- Prompt -->
          <el-form-item :label="t('videoGeneration.prompt')">
            <el-input
              v-model="form.prompt"
              type="textarea"
              :rows="4"
              :placeholder="t('videoGeneration.describeTheVideoYouWantToCreate')"
            />
          </el-form-item>

          <!-- Reference Image -->
          <el-form-item :label="t('videoGeneration.referenceImage')">
            <div class="image-upload-area" @click="triggerUpload">
              ...
              <div v-if="!form.imageUrl" class="upload-placeholder">
                ...
                <span>{{ t('videoGeneration.clickToUploadImage') }}</span>
              </div>
          ...
          <!-- Resolution -->
          <el-form-item :label="t('videoGeneration.resolution')">
      ...
      <section class="gallery-panel">
        <div class="gallery-header">
          <div class="header-title">
            ...
            <span>{{ t('videoGeneration.videoGallery') }}</span>
            <el-tag type="info">{{ pagination.total }} {{ t('videoGeneration.videos') }}</el-tag>
          </div>
        ...
        <div v-if="loadingTasks" class="loading-state">
          ...
          <span>{{ t('videoGeneration.loadingVideos') }}</span>
        </div>

        <!-- Empty State -->
        <div v-else-if="tasks.length === 0" class="empty-state">
          <el-icon><VideoCamera /></el-icon>
          <p>{{ t('videoGeneration.noVideosYet') }}</p>
          <span>{{ t('videoGeneration.generateYourFirstVideoToSeeItHere') }}</span>
        </div>
        ...
        <el-table v-else :data="tasks" class="video-table">
          <el-table-column :label="t('videoGeneration.video')" width="120">
          ...
          <el-table-column prop="prompt" :label="t('videoGeneration.prompt')">
            <template #default="{ row }">
              <el-text truncated>{{ row.prompt }}</el-text>
            </template>
          </el-table-column>
          <el-table-column prop="status" :label="t('videoGeneration.status')" width="120">
            <template #default="{ row }">
          ...
          <el-table-column prop="created_at" :label="t('videoGeneration.created')" width="150">
            <template #default="{ row }">{{ formatDate(row.created_at) }}</template>
          </el-table-column>
          <el-table-column :label="t('videoGeneration.actions')" width="100">
            <template #default="{ row }">
              <el-button 
    ...
    <el-dialog 
      ...
      :title="selectedTask?.prompt || t('videoGeneration.videoPreview')"
      width="70%"
      ...
      <div class="player-content">
        ...
        <div v-else-if="selectedTask?.status === 'processing'" class="processing-state">
          <el-progress 
            ...
            :width="120"
          />
          <p>{{ t('videoGeneration.videoIsBeingGenerated') }}</p>
        </div>
      </div>
      <div class="player-info">
        <el-descriptions :column="3" border>
          <el-descriptions-item :label="t('videoGeneration.provider')">{{ selectedTask?.provider }}</el-descriptions-item>
          <el-descriptions-item :label="t('videoGeneration.model')">{{ selectedTask?.model }}</el-descriptions-item>
          <el-descriptions-item :label="t('videoGeneration.resolution')">{{ selectedTask?.resolution || 'N/A' }}</el-descriptions-item>
          <el-descriptions-item :label="t('videoGeneration.status')">
            <el-tag :type="getStatusType(selectedTask?.status || '')">{{ selectedTask?.status }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item :label="t('videoGeneration.created')">{{ formatDate(selectedTask?.created_at || '') }}</el-descriptions-item>
          <el-descriptions-item :label="t('videoGeneration.fileSize')">{{ formatFileSize(selectedTask?.file_size) }}</el-descriptions-item>
        </el-descriptions>
      </div>
    </el-dialog>
  </div>
