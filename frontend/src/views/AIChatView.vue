  ...
  <div class="ai-chat-view">
    ...
    <div class="header">
      <div class="header-left">
        ...
        <div class="mode-switcher">
          <router-link to="/" class="mode-btn">{{ t('aiChat.aiHumanChat') }}</router-link>
          <span class="mode-btn active">{{ t('aiChat.aiAiChat') }}</span>
        </div>
      </div>
      <div class="header-right">
        <button class="btn btn-secondary" @click="$router.push('/programming')">
          {{ t('aiChat.aiProgramming') }}
        </button>
        <button class="btn btn-secondary" @click="$router.push('/video')">
          {{ t('aiChat.videoGeneration') }}
        </button>
        <button class="btn btn-secondary" @click="showTemplates = true">
          {{ t('aiChat.selectTemplate') }}
        </button>
    ...
    <div class="main-content">
      ...
      <aside class="session-history" :class="{ collapsed: historyCollapsed }">
        <div class="history-header" v-show="!historyCollapsed">
          <h4>{{ t('aiChat.historySessions') }}</h4>
      ...
      <div class="config-panel" v-if="showConfig && !isRunning">
        ...
        <div class="panel-body">
          ...
          <div class="config-section">
            <h4>{{ t('aiChat.basicSettings') }}</h4>
            <div class="form-group">
              <label>{{ t('aiChat.sessionTitle') }}</label>
              <input v-model="sessionConfig.title" :placeholder="t('aiChat.sessionTitlePlaceholder')" />
            </div>
            <div class="form-group">
              <label>{{ t('aiChat.discussionTopic') }}</label>
              <textarea v-model="sessionConfig.topic" :placeholder="t('aiChat.discussionTopicPlaceholder')" rows="2" />
            </div>
            <div class="form-group">
              <label>{{ t('aiChat.globalConstraint') }}</label>
              <textarea v-model="sessionConfig.global_constraint" :placeholder="t('aiChat.globalConstraintPlaceholder')" rows="2" />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label>{{ t('aiChat.maxRounds') }}</label>
                <input type="number" v-model.number="sessionConfig.max_rounds" min="1" max="50" />
              </div>
              <div class="form-group">
                <label>{{ t('aiChat.roundDelay') }}</label>
          ...
          <!-- AI-A 配置 -->
          <div class="config-section">
            <h4>{{ t('aiChat.aiAConfig') }}</h4>
            <div class="form-group">
              <label>{{ t('aiChat.name') }}</label>
              <input v-model="sessionConfig.agent_a.name" :placeholder="t('aiChat.namePlaceholder')" />
            </div>
            <div class="form-group">
              <label>{{ t('aiChat.roleSetting') }}</label>
              <textarea v-model="sessionConfig.agent_a.role" :placeholder="t('aiChat.roleSettingPlaceholder')" rows="3" />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label>{{ t('aiChat.languageStyle') }}</label>
                <select v-model="sessionConfig.agent_a.style.language_style">
                  <option value="professional">{{ t('aiChat.professional') }}</option>
                  <option value="casual">{{ t('aiChat.casual') }}</option>
                  <option value="poetic">{{ t('aiChat.poetic') }}</option>
                  <option value="academic">{{ t('aiChat.academic') }}</option>
                  <option value="humorous">{{ t('aiChat.humorous') }}</option>
                </select>
              </div>
              <div class="form-group">
                <label>{{ t('aiChat.knowledgeLevel') }}</label>
                <select v-model="sessionConfig.agent_a.style.knowledge_level">
                  <option value="beginner">{{ t('aiChat.beginner') }}</option>
                  <option value="intermediate">{{ t('aiChat.intermediate') }}</option>
                  <option value="expert">{{ t('aiChat.expert') }}</option>
                </select>
              </div>
            </div>
            <div class="form-group">
              <label>{{ t('aiChat.tone') }}</label>
              <input v-model="sessionConfig.agent_a.style.tone" :placeholder="t('aiChat.tonePlaceholder')" />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label>{{ t('aiChat.aiModel') }}</label>
                <select v-model="sessionConfig.agent_a.model">
                  <option v-for="m in availableModels" :key="m.id" :value="m.id">{{ m.provider }} / {{ m.name }}</option>
                </select>
              </div>
              <div class="form-group">
                <label>{{ t('aiChat.temperature') }}</label>
          ...
          <!-- AI-B 配置 -->
          <div class="config-section">
            <h4>{{ t('aiChat.aiBConfig') }}</h4>
            <div class="form-group">
              <label>{{ t('aiChat.name') }}</label>
              <input v-model="sessionConfig.agent_b.name" :placeholder="t('aiChat.namePlaceholder')" />
            </div>
            <div class="form-group">
              <label>{{ t('aiChat.roleSetting') }}</label>
              <textarea v-model="sessionConfig.agent_b.role" :placeholder="t('aiChat.roleSettingPlaceholder')" rows="3" />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label>{{ t('aiChat.languageStyle') }}</label>
            ...
            <div class="form-group">
              <label>{{ t('aiChat.tone') }}</label>
              <input v-model="sessionConfig.agent_b.style.tone" :placeholder="t('aiChat.tonePlaceholder')" />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label>{{ t('aiChat.aiModel') }}</label>
          ...
          <div v-if="sessionConfig.agents.length > 0" class="config-section">
            <h4>{{ t('aiChat.additionalAgents') }}</h4>
            <div v-for="(agent, index) in sessionConfig.agents" :key="index" class="agent-card">
              ...
              <div class="form-group">
                <label>{{ t('aiChat.name') }}</label>
                <input v-model="agent.name" :placeholder="t('aiChat.namePlaceholder')" />
              </div>
              <div class="form-group">
                <label>{{ t('aiChat.roleSetting') }}</label>
                <textarea v-model="agent.role" :placeholder="t('aiChat.roleSettingPlaceholder')" rows="2" />
              </div>
              <div class="form-row">
                <div class="form-group">
                  <label>{{ t('aiChat.aiModel') }}</label>
          ...
          <div class="add-agent-section">
            ...
            <p class="add-agent-hint" v-if="sessionConfig.agents.length === 0">
              {{ t('aiChat.multiAgentHint') }}
            </p>
          </div>

          <!-- 终止条件 -->
          <div class="config-section">
            <h4>{{ t('aiChat.terminationConditions') }}</h4>
            <div class="form-group">
              <label>{{ t('aiChat.terminationType') }}</label>
              <select v-model="sessionConfig.termination_config.type">
                <option value="fixed_rounds">{{ t('aiChat.fixedRounds') }}</option>
                <option value="open_ended">{{ t('aiChat.openEnded') }}</option>
                <option value="keyword">{{ t('aiChat.keywordTriggered') }}</option>
              </select>
            </div>
            <div class="form-group" v-if="sessionConfig.termination_config.type === 'keyword'">
              <label>{{ t('aiChat.terminationKeywords') }}</label>
              <input v-model="terminationKeywords" :placeholder="t('aiChat.terminationKeywordsPlaceholder')" />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label>{{ t('aiChat.similarityThreshold') }}</label>
              ...
              <div class="form-group">
                <label>{{ t('aiChat.consecutiveSimilarRounds') }}</label>
        ...
        <div class="panel-footer">
          <button class="btn btn-secondary" @click="resetConfig">{{ t('aiChat.reset') }}</button>
          <button class="btn btn-primary" @click="saveConfig">{{ t('aiChat.saveConfig') }}</button>
        </div>
      </div>
      ...
      <div class="chat-area" :class="{ 'with-monitor': showMonitor }">
        ...
        <div class="control-bar">
          <div class="status-info">
            ...
            <span class="round-info">{{ t('aiChat.roundN', { round: currentRound }) }}/{{ maxRounds }} {{ t('aiChat.round') }}</span>
          ...
          <div class="control-buttons">
            ...
            <button class="btn btn-secondary" @click="showMonitor = !showMonitor">
              📊 {{ showMonitor ? t('aiChat.hide') : t('aiChat.show') }}{{ t('aiChat.monitor') }}
            </button>
            <el-dropdown @command="exportSession" trigger="click" :disabled="!sessionId">
              <button class="btn btn-secondary" :disabled="!sessionId">
                📥 {{ t('aiChat.export') }} <el-icon style="vertical-align: middle;"><ArrowDown /></el-icon>
              </button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="markdown">{{ t('aiChat.exportMarkdown') }}</el-dropdown-item>
                  <el-dropdown-item command="json">{{ t('aiChat.exportJson') }}</el-dropdown-item>
                  <el-dropdown-item command="text">{{ t('aiChat.exportText') }}</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
        ...
        <div class="director-panel" v-if="isRunning || isPaused">
          <div class="director-input">
            <select v-model="directorTarget">
              <option value="agent_a">{{ t('aiChat.toAgentA') }}</option>
              <option value="agent_b">{{ t('aiChat.toAgentB') }}</option>
              <option value="both">{{ t('aiChat.toBoth') }}</option>
            </select>
            <input 
              v-model="directorCommand" 
              :placeholder="t('aiChat.directorCommandPlaceholder')" 
              @keyup.enter="sendDirectorCommand"
            />
            <button class="btn btn-primary" @click="sendDirectorCommand">{{ t('aiChat.sendCommand') }}</button>
          </div>
        </div>
        ...
        <div class="message-list" ref="messageList" v-show="viewMode === 'list'">
          ...
          >
            <div class="message-header">
              ...
              <span class="message-round">{{ t('aiChat.roundN', { round: message.round }) }}</span>
        ...
        <div class="empty-state" v-if="messages.length === 0">
          ...
          <p>{{ t('aiChat.emptyStateMessage') }}</p>
          <button class="btn btn-primary" @click="showConfig = true">{{ t('aiChat.configureSession') }}</button>
        </div>
      </div>
      ...
      <div class="monitor-panel" v-if="showMonitor">
        <div class="panel-header">
          <h4>{{ t('aiChat.realtimeMonitor') }}</h4>
        </div>
        <div class="panel-body">
          <!-- Token 使用 -->
          <div class="monitor-section">
            <h5>{{ t('aiChat.tokenUsage') }}</h5>
            <div class="token-bar">
          ...
          <!-- 对话流程 -->
          <div class="monitor-section">
            <h5>{{ t('aiChat.conversationFlow') }}</h5>
          ...
          <!-- 统计信息 -->
          <div class="monitor-section">
            <h5>{{ t('aiChat.statistics') }}</h5>
            <div class="stats-grid">
              <div class="stat-item">
                <span class="stat-label">{{ t('aiChat.totalMessages') }}</span>
                <span class="stat-value">{{ messages.length }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">{{ t('aiChat.runningTime') }}</span>
                <span class="stat-value">{{ runningTime }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">{{ t('aiChat.avgLatency') }}</span>
          ...
          <div class="monitor-section" v-if="evaluation || evaluationLoading">
            ...
            <div class="evaluation-highlights" v-if="evaluation.highlights && evaluation.highlights.length">
              <div class="stat-label">{{ t('aiChat.highlights') }}</div>
              <ul>
                <li v-for="(h, idx) in evaluation.highlights" :key="idx">
                  <strong>{{ t('aiChat.roundN', { round: h.round }) }} - {{ h.agent_name }}：</strong>{{ h.content }}
                </li>
              </ul>
            </div>
            </template>
    ...
    <div class="modal" v-if="showTemplates" @click.self="showTemplates = false">
      <div class="modal-content modal-content-wide">
        <div class="modal-header">
          <h3>{{ t('aiChat.templateLibrary') }}</h3>
          <div class="modal-header-actions">
            <button class="btn btn-primary btn-sm" @click="openCreateTemplate">{{ t('aiChat.createTemplate') }}</button>
            ...
            >
              <div class="template-card-body" @click="applyTemplate(tmpl)">
                ...
                <div class="template-info">
                  ...
                  <span class="template-badge builtin" v-if="tmpl.is_builtin">{{ t('aiChat.builtin') }}</span>
                  <span class="template-badge custom" v-else>{{ t('aiChat.custom') }}</span>
                </div>
              </div>
              <div class="template-card-actions">
                <button class="tpl-btn" @click.stop="cloneTemplateAction(tmpl.id)" :title="t('aiChat.cloneTemplate')">📋</button>
                <button class="tpl-btn" @click.stop="openEditTemplate(tmpl)" v-if="!tmpl.is_builtin" :title="t('aiChat.edit')">✏️</button>
                <button class="tpl-btn tpl-btn-danger" @click.stop="deleteTemplateConfirm(tmpl.id)" v-if="!tmpl.is_builtin" :title="t('aiChat.delete')">🗑️</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="modal" v-if="showTemplateEditor" @click.self="showTemplateEditor = false">
      <div class="modal-content modal-content-editor">
        <div class="modal-header">
          <h3>{{ templateEditorMode === 'create' ? t('aiChat.createTemplate') : t('aiChat.editTemplate') }}</h3>
          <button class="close-btn" @click="showTemplateEditor = false">×</button>
        </div>
        <div class="modal-body editor-body">
          <!-- 基本信息 -->
          <div class="editor-section">
            <h4 class="editor-section-title">{{ t('aiChat.basicInfo') }}</h4>
            <div class="editor-row">
              <div class="editor-field">
                <label>{{ t('aiChat.templateName') }}</label>
                <input v-model="templateForm.name" :placeholder="t('aiChat.templateNamePlaceholder')" />
              </div>
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.icon') }}</label>
                <input v-model="templateForm.icon" :placeholder="t('aiChat.iconPlaceholder')" maxlength="4" />
              </div>
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.category') }}</label>
                <select v-model="templateForm.category">
                  <option value="debate">{{ t('aiChat.debate') }}</option>
                  <option value="interview">{{ t('aiChat.interview') }}</option>
                  <option value="collaboration">{{ t('aiChat.collaboration') }}</option>
                  <option value="roleplay">{{ t('aiChat.roleplay') }}</option>
                  <option value="creative">{{ t('aiChat.creative') }}</option>
                  <option value="education">{{ t('aiChat.education') }}</option>
                  <option value="custom">{{ t('aiChat.custom') }}</option>
                </select>
              </div>
            </div>
            <div class="editor-field">
              <label>{{ t('aiChat.description') }}</label>
              <textarea v-model="templateForm.description" rows="2" :placeholder="t('aiChat.descriptionPlaceholder')" />
            </div>
          </div>

          <!-- 对话设置 -->
          <div class="editor-section">
            <h4 class="editor-section-title">{{ t('aiChat.conversationSettings') }}</h4>
            <div class="editor-field">
              <label>{{ t('aiChat.discussionTopic') }} *</label>
              <input v-model="templateForm.config.topic" :placeholder="t('aiChat.discussionTopicPlaceholder')" />
            </div>
            <div class="editor-field">
              <label>{{ t('aiChat.globalConstraints') }}</label>
              <textarea v-model="templateForm.config.global_constraint" rows="2" :placeholder="t('aiChat.globalConstraintsPlaceholder')" />
            </div>
            <div class="editor-row">
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.maxRounds') }}</label>
              ...
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.terminationMethod') }}</label>
                <select v-model="templateForm.config.termination_config.type">
                  <option value="fixed_rounds">{{ t('aiChat.fixedRounds') }}</option>
                  <option value="keyword">{{ t('aiChat.keywordDetection') }}</option>
                  <option value="open_ended">{{ t('aiChat.openEndedTermination') }}</option>
                </select>
              </div>
              <div class="editor-field editor-field-sm" v-if="templateForm.config.termination_config.type === 'keyword'">
                <label>{{ t('aiChat.terminationKeywords') }}</label>
                <input v-model="templateForm.config.termination_config.keywords_str" :placeholder="t('aiChat.terminationKeywordsPlaceholder')" />
              </div>
            </div>
          </div>
          ...
          <!-- AI-A 配置 -->
          <div class="editor-section">
            <h4 class="editor-section-title">{{ t('aiChat.aiAConfig') }}</h4>
            <div class="editor-row">
              <div class="editor-field">
                <label>{{ t('aiChat.name') }}</label>
                <input v-model="templateForm.config.agent_a.name" :placeholder="t('aiChat.namePlaceholder')" />
              </div>
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.aiModel') }}</label>
                <select v-model="templateForm.config.agent_a.model">
                  <option value="">{{ t('aiChat.default') }}</option>
            ...
            <div class="editor-field">
              <label>{{ t('aiChat.roleSetting') }}</label>
              <textarea v-model="templateForm.config.agent_a.role" rows="3" :placeholder="t('aiChat.roleSettingPlaceholder')" />
            </div>
            <div class="editor-row">
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.languageStyle') }}</label>
              ...
              <div class="editor-field">
                <label>{{ t('aiChat.tone') }}</label>
                <input v-model="templateForm.config.agent_a.style.tone" :placeholder="t('aiChat.tonePlaceholder')" />
              </div>
            </div>
          </div>
          ...
          <!-- AI-B 配置 -->
          <div class="editor-section">
            <h4 class="editor-section-title">{{ t('aiChat.aiBConfig') }}</h4>
            <div class="editor-row">
              <div class="editor-field">
                <label>{{ t('aiChat.name') }}</label>
                <input v-model="templateForm.config.agent_b.name" :placeholder="t('aiChat.namePlaceholder')" />
              </div>
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.aiModel') }}</label>
            ...
            <div class="editor-field">
              <label>{{ t('aiChat.roleSetting') }}</label>
              <textarea v-model="templateForm.config.agent_b.role" rows="3" :placeholder="t('aiChat.roleSettingPlaceholder')" />
            </div>
            <div class="editor-row">
              <div class="editor-field editor-field-sm">
                <label>{{ t('aiChat.languageStyle') }}</label>
              ...
              <div class="editor-field">
                <label>{{ t('aiChat.tone') }}</label>
                <input v-model="templateForm.config.agent_b.style.tone" :placeholder="t('aiChat.tonePlaceholder')" />
              </div>
            </div>
          </div>
          ...
          <div class="editor-footer">
            <button class="btn btn-secondary" @click="showTemplateEditor = false">{{ t('aiChat.cancel') }}</button>
            <button class="btn btn-primary" @click="saveTemplateEditor" :disabled="!templateForm.name || !templateForm.config.topic">
              {{ templateEditorMode === 'create' ? t('aiChat.createTemplateBtn') : t('aiChat.saveChanges') }}
            </button>
          </div>
        </div>
      </div>
