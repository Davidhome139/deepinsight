  ...
  <div class="knowledge-base-view">
    <div class="header">
      <div class="header-left">
        <button class="back-btn" @click="goBack" :title="t('knowledgeBase.backToChat')">
          ← {{ t('knowledgeBase.back') }}
        </button>
        <h1>{{ t('knowledgeBase.knowledgeBase') }}</h1>
      </div>
      <button class="upload-btn" @click="showUpload = true">
        <span>📤</span> {{ t('knowledgeBase.uploadDocument') }}
      </button>
    </div>
    ...
    <div v-if="showUpload" class="modal-overlay" @click.self="showUpload = false">
      <div class="modal">
        <h2>{{ t('knowledgeBase.uploadDocument') }}</h2>
        <div 
          class="drop-zone"
        ...
        >
          ...
          <div class="drop-content">
            ...
            <p>{{ t('knowledgeBase.dropFileHereOrClickToSelect') }}</p>
            <p class="file-types">{{ t('knowledgeBase.supported') }}: TXT, MD, PDF, DOCX</p>
          </div>
        </div>
        ...
        <div class="modal-actions">
          <button class="cancel-btn" @click="showUpload = false">{{ t('knowledgeBase.cancel') }}</button>
          <button 
    ...
    <div class="documents-section">
      <h2>{{ t('knowledgeBase.documents') }}</h2>
      <div v-if="loading" class="loading">{{ t('knowledgeBase.loadingDocuments') }}</div>
      <div v-else-if="documents.length === 0" class="empty">
        <p>{{ t('knowledgeBase.noDocumentsUploadedYet') }}</p>
        <p class="hint">{{ t('knowledgeBase.uploadDocumentsToBuildYourKnowledgeBase') }}</p>
      </div>
      <div v-else class="documents-grid">
        <div v-for="doc in documents" :key="doc.id" class="document-card">
          ...
          <div class="doc-actions">
            <button @click="viewChunks(doc)" :title="t('knowledgeBase.viewChunks')">📋</button>
            <button @click="deleteDocument(doc.id)" :title="t('knowledgeBase.delete')" class="delete-btn">🗑️</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="search-section">
      <h2>{{ t('knowledgeBase.searchKnowledgeBase') }}</h2>
      <p class="search-hint">{{ t('knowledgeBase.testYourKnowledgeBaseByEnteringAQuery') }}</p>
      <div class="search-input-wrapper">
        ...
        <button @click="performSearch" :disabled="!searchQuery || searching || !hasReadyDocuments">
          {{ searching ? '🔄' : '🔍' }} {{ t('knowledgeBase.search') }}
        </button>
      </div>
      <p v-if="!hasReadyDocuments" class="search-warning">⚠️ {{ t('knowledgeBase.uploadAndWaitForDocumentsToBeReadyBeforeSearching') }}</p>

      <div v-if="searchResults.length > 0" class="search-results">
        <h3>{{ t('knowledgeBase.results', { count: searchResults.length }) }}</h3>
        <div v-for="result in searchResults" :key="result.chunk_id" class="result-item">
          <div class="result-header">
            <span class="result-file">{{ result.filename }}</span>
            <span class="result-score">{{ (result.score * 100).toFixed(1) }}% {{ t('knowledgeBase.match') }}</span>
          </div>
    ...
    <div v-if="showChunks" class="modal-overlay" @click.self="showChunks = false">
      <div class="modal chunks-modal">
        <h2>{{ t('knowledgeBase.documentChunks') }}</h2>
        <div class="chunks-list">
          <div v-for="chunk in chunks" :key="chunk.id" class="chunk-item">
            <div class="chunk-header">
              <span>{{ t('knowledgeBase.chunk') }} #{{ chunk.chunk_index + 1 }}</span>
              <span class="token-count">{{ chunk.token_count }} {{ t('knowledgeBase.tokens') }}</span>
            </div>
            <p class="chunk-content">{{ chunk.content }}</p>
          </div>
        </div>
        <button class="close-btn" @click="showChunks = false">{{ t('knowledgeBase.close') }}</button>
      </div>
    </div>
  </div>
</template>
