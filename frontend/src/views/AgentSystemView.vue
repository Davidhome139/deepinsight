  ...
  <div class="agent-system-view">
    ...
    <div class="nav-sidebar">
      <div class="nav-header">
        <h2>🤖 {{ t('agentSystem.agentStudio') }}</h2>
      </div>
      
      <el-menu :default-active="activeSection" @select="handleNavSelect">
        <el-menu-item index="agents">
          <el-icon><User /></el-icon>
          <span>{{ t('agentSystem.myAgents') }}</span>
        </el-menu-item>
        <el-menu-item index="workflows">
          <el-icon><Connection /></el-icon>
          <span>{{ t('agentSystem.workflows') }}</span>
        </el-menu-item>
        <el-menu-item index="permissions">
          <el-icon><Lock /></el-icon>
          <span>{{ t('agentSystem.permissions') }}</span>
        </el-menu-item>
        <el-menu-item index="marketplace">
          <el-icon><Shop /></el-icon>
          <span>{{ t('agentSystem.marketplace') }}</span>
        </el-menu-item>
        <el-menu-item index="ab-tests">
          <el-icon><DataAnalysis /></el-icon>
          <span>{{ t('agentSystem.abTests') }}</span>
        </el-menu-item>
      </el-menu>
    </div>
    ...
    <div class="main-content">
      ...
      <template v-if="activeSection === 'agents'">
        ...
        <div v-else class="section-content">
          <div class="section-header">
            <h1>{{ t('agentSystem.myAgents') }}</h1>
            <el-button type="primary" @click="createNewAgent">
              <el-icon><Plus /></el-icon> {{ t('agentSystem.createAgent') }}
            </el-button>
          </div>

          <div class="agents-grid">
            <el-empty v-if="agents.length === 0" :description="t('agentSystem.noAgentsYet')">
              <el-button type="primary" @click="createNewAgent">{{ t('agentSystem.createYourFirstAgent') }}</el-button>
            </el-empty>
            ...
            <div v-for="agent in agents" :key="agent.id" class="agent-card">
              <div class="card-header">
                ...
                <el-dropdown @command="handleAgentCommand($event, agent)">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item command="edit">{{ t('agentSystem.edit') }}</el-dropdown-item>
                      <el-dropdown-item command="duplicate">{{ t('agentSystem.duplicate') }}</el-dropdown-item>
                      <el-dropdown-item command="export">{{ t('agentSystem.export') }}</el-dropdown-item>
                      <el-dropdown-item command="publish">{{ t('agentSystem.publish') }}</el-dropdown-item>
                      <el-dropdown-item command="delete" divided>{{ t('agentSystem.delete') }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
              </div>
              ...
              <div class="card-footer">
                <el-button size="small" @click="testAgent(agent)">{{ t('agentSystem.test') }}</el-button>
                <el-button type="primary" size="small" @click="editAgent(agent)">{{ t('agentSystem.edit') }}</el-button>
              </div>
            </div>
          </div>
        </div>
      ...
      <template v-else-if="activeSection === 'workflows'">
        ...
        <div v-else class="section-content">
          <div class="section-header">
            <h1>{{ t('agentSystem.workflows') }}</h1>
            <el-button type="primary" @click="createNewWorkflow">
              <el-icon><Plus /></el-icon> {{ t('agentSystem.createWorkflow') }}
            </el-button>
          </div>

          <div class="workflows-grid">
            <el-empty v-if="workflows.length === 0" :description="t('agentSystem.noWorkflowsYet')">
              <el-button type="primary" @click="createNewWorkflow">{{ t('agentSystem.createYourFirstWorkflow') }}</el-button>
            </el-empty>
            ...
            <div v-for="workflow in workflows" :key="workflow.id" class="workflow-card">
              ...
              <div class="card-footer">
                <el-button size="small" @click="runWorkflow(workflow)">{{ t('agentSystem.run') }}</el-button>
                <el-button type="primary" size="small" @click="editWorkflow(workflow)">{{ t('agentSystem.edit') }}</el-button>
              </div>
            </div>
          </div>
        </div>
      ...
      <template v-else-if="activeSection === 'ab-tests'">
        <div class="section-content">
          <div class="section-header">
            <h1>{{ t('agentSystem.abTests') }}</h1>
            <el-button type="primary" @click="showABTestDialog = true">
              <el-icon><Plus /></el-icon> {{ t('agentSystem.newTest') }}
            </el-button>
          </div>

          <el-empty v-if="abTests.length === 0" :description="t('agentSystem.noAbTestsYet')">
            <p>{{ t('agentSystem.compareDifferentAgentVersions') }}</p>
          </el-empty>
            ...
            <div v-for="test in abTests" :key="test.id" class="ab-test-card">
              ...
              <div class="test-footer">
                <el-button v-if="test.status === 'draft'" type="primary" size="small" @click="startTest(test)">
                  {{ t('agentSystem.startTest') }}
                </el-button>
                <el-button size="small" @click="viewTestDetails(test)">{{ t('agentSystem.viewDetails') }}</el-button>
              </div>
            </div>
          </div>
        </div>
    ...
    <el-dialog v-model="showABTestDialog" :title="t('agentSystem.createAbTest')" width="500px">
      <el-form :model="abTestForm" label-position="top">
        <el-form-item :label="t('agentSystem.testName')">
          <el-input v-model="abTestForm.name" :placeholder="t('agentSystem.testNamePlaceholder')" />
        </el-form-item>
        <el-form-item :label="t('agentSystem.type')">
          <el-select v-model="abTestForm.type" style="width: 100%">
            <el-option :label="t('agentSystem.agent')" value="agent" />
            <el-option :label="t('agentSystem.workflow')" value="workflow" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('agentSystem.variantA')">
        ...
        <el-form-item :label="t('agentSystem.variantB')">
        ...
        <el-form-item :label="t('agentSystem.minSampleSize')">
          <el-input-number v-model="abTestForm.min_sample_size" :min="10" :max="1000" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showABTestDialog = false">{{ t('agentSystem.cancel') }}</el-button>
        <el-button type="primary" @click="createABTest" :loading="creatingTest">{{ t('agentSystem.create') }}</el-button>
      </template>
    </el-dialog>

    <!-- Test Agent Dialog -->
    <el-dialog v-model="showTestDialog" :title="t('agentSystem.testAgent')" width="600px">
      <div class="test-agent-dialog">
        ...
        <div class="test-input">
          <el-input v-model="testInput" :placeholder="t('agentSystem.enterTestMessage')" @keyup.enter="sendTestMessage">
            <template #append>
              <el-button @click="sendTestMessage" :loading="testLoading">{{ t('agentSystem.send') }}</el-button>
            </template>
          </el-input>
        </div>
      </div>
